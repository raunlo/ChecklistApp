package com.raunlo.baseItemList.persistence.dao;
...
public interface ChecklistDao {
    ...
    Optional<ChecklistDbo> getChecklistdbo(@Bind("id") Long checklistId);
    ...
    class ChecklistRowReducer implements LinkedHashMapRowReducer<Long, ChecklistDbo> {
        ...
        public void accumulate(Map<Long, ChecklistDbo> container, RowView rowView) {
            final ChecklistDbo baseItemList = container.computeIfAbsent(rowView.getColumn("checklist_id", Long.class),
                    id -> rowView.getRow(ChecklistDbo.class));

            if (rowView.getColumn("task_id", Long.class) != null) {
                baseItemList.addTask(rowView.getRow(TaskDbo.class));
            }
        }
    }
